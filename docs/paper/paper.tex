%% Digital Chemical Engineering (Elsevier) — CAS double-column manuscript.
...
[[quantified in Section~\ref{sec:validation}]] and illustrated on a
flowsheet-level case study [[Section~\ref{sec:casestudy}, pending]].
...
the stage cut by more than six percent at elevated pressure. Real-gas behaviour
...
because there is no single standard module geometry. The same limitation is
...
must be added through a Visual Basic or C++ subroutine, and by
...
\citep{alqaheem2024accuracy}. A third route keeps the calculation in an external
...
form of Eq.~\eqref{eq:flux}.

\subsection{Local permeate composition and module balances}\label{sec:model-balances}
The permeate leaving the active layer at a given position is composed of the
local fluxes, so its mole fractions are the flux fractions,
$y_i = J_i / \sum_k J_k$. Writing $S_i = Q_i/Q_1$ for the permeance ratios and
$\gamma = p_\mathrm{p}/p_\mathrm{r}$ for the pressure ratio, this becomes a
coupled implicit set in the permeate composition,
...
classical binary local permeate relationship \citep{weller1950} and is solved at
every position by the reformulated solver of Section~\ref{sec:numerics}. The
local total flux follows as
$J_\mathrm{tot} = \sum_k Q_k (p_\mathrm{r} x_k - p_\mathrm{p} y_k)$.

Integrating the local behaviour over the module, with area $a$ measured from the
feed end and $R(a)$ the local retentate molar flow, gives the retentate-side
balances

\begin{equation}\label{eq:mass-balance}
-\,\frac{\mathrm{d}R}{\mathrm{d}a} = J_\mathrm{tot}(a), \qquad
-\,\frac{\mathrm{d}(R\,x_i)}{\mathrm{d}a} = y_i(a)\,J_\mathrm{tot}(a).
\end{equation}

The overall stage cut, the single most important design variable, is the
fraction of the feed that permeates,
...
spiral-wound modules \citep{coker1998}, the permeate formed at a point leaves
...
The separation is computed isothermally, because the permeance is taken
temperature independent and the driving force at fixed pressure and composition
does not depend on temperature. An optional adiabatic energy balance is then
...
state implies; the unit does not hard-code a caloric correlation. A
...
temperature, pressure and composition), a negligible pressure drop along the
retentate channel, and no back-permeation, so a component whose local driving
force would become negative is capped at zero flux. The real-gas correction is
applied to the driving force only and not to the density and velocity balances or
to a channel pressure drop. The consequences of these assumptions are examined in
...
ill-conditioned as the pressure ratio tends to zero, where the common denominator
collapses. The implementation instead reduces the problem to a single scalar
...
Because $S_k$, $x_k$, $\gamma$ and $t$ are non-negative, every denominator is
strictly positive and the singularity is removed. The function $H(t)$ is strictly
decreasing, is positive at $t \to 0^{+}$ and negative at $t = \sum_k S_k x_k$, so
the root is unique and bracketed in $\bigl(0, \sum_k S_k x_k\bigr)$. It is solved
...
then follows explicitly from Eq.~\eqref{eq:yk-t}.
...
set well beyond the point of diminishing returns; the default reproduces the
Shindo benchmark to a relative agreement of order $10^{-4}$
...
$4.80\times10^{-10}$ rather than the $4.80\times10^{-9}$ printed in the source
table \citep{diaspinto2020}, which is a factor-of-ten typographical error: with
the printed value the reported enrichment is unreachable, whereas the corrected
value reproduces the benchmark and preserves the expected ordering of the
microporous-glass permeances.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{val_shindo_case1}
...
above. Cases 3 and 4 give no tabulated outputs, so the unit runs them as
application checks. Case 3, a carbon dioxide removal from natural gas through
cellulose acetate at a pressure ratio of $0.04$, gives a leading-edge permeate of
about 86\,\% carbon dioxide from an 18\,\% feed. Case 4, a carbon dioxide capture
from flue gas through a commercial membrane at a pressure ratio of $0.12$,
reaches about 90\,\% carbon dioxide recovery at a stage cut of $0.36$, which
matches the recovery reported in the source.
...
track the digitised reference curves with a mean absolute deviation of about
$0.002$ mole fraction, the largest deviation being about $0.008$ near a stage cut
of $0.60$ at the lowest feed pressure (Fig.~\ref{fig:aziaba}). For the Chowdhury
...
components, provided the argon permeance is taken as $7.0$ rather than the $70$
printed in the source table, again a factor-of-ten typographical error.
...
For a carbon dioxide and methane cross-flow at fixed membrane area, using
Peng--Robinson fugacity coefficients on the retentate and permeate sides, the
ideal-gas partial-pressure assumption progressively over-predicts the stage cut
as the feed pressure rises and the fugacity coefficient falls below unity, by
about 9\,\% at 60 bar and about 15\,\% at 100 bar (Fig.~\ref{fig:igrg}). This is
...
relative fraction of the smaller real-gas value, and the two differ by roughly a
factor of two. Reported in the absolute convention of the source, the
...
captures the correct sign and trend and removes about 40 to 45\,\% of the
ideal-gas error, which is a useful screening-level improvement.
...
\begin{figure}[t]
  ...
  result and the analytic line. The permeate carries all the temperature change
  and the retentate none.}\label{fig:energy}
\end{figure}

%% ============================================================================
\section{Illustrative case study}\label{sec:casestudy}
% ===================== PARKED — DECISION PENDING =============================
% Candidate showcases: (a) CO2/CH4 NG sweetening at high pressure;
% (b) H2 recovery/purification; (c) O2/N2 air separation;
% (d) multi-stage with recycle + compression.
% Goal: demonstrate in-simulator value (membrane solved with compressors/
% exchangers using the host thermodynamics). Flowsheet + results to follow.
[[Case study pending; see repository \texttt{docs/paper/outline.md}.]]
...
for membranes with strongly composition-dependent or plasticising behaviour. The
...
cross-flow solution to an absolute agreement of order $10^{-4}$, matches the
spiral-wound cases of Dias and the hollow-fibre counter-current data compiled by
Aziaba, and tracks a twelve-point air-separation experiment to within $0.010$
mole fraction. The real-gas driving force captures the high-pressure deviation
from ideal-gas behaviour that a partial-pressure model misses. By combining an
