...
worst = np.max(np.abs(pred - meas))
ax.set_title("Air separation vs. experiment (DeJaco et al., MemPy): 12 points, "
             f"max $|\\Delta y_{{O_2}}|$={worst:.3f}", color=NAVY, fontsize=9.5)
fig.tight_layout()
fig.savefig(os.path.join(HERE, "val_airsep.png"), dpi=150)
